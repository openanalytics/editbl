...
tableUI <- function(id){
	...
	fluidPage(
		eDTOutput(id = ns('DT')),
		actionButton(inputId = ns('refresh'), label = 'refresh', icon = shiny::icon('arrow-rotate-left'))
	)
}

tableServer <- function(id, table, dm) {
  moduleServer(
      id,
      function(input, output, session) {
		...
		observeEvent(input$refresh,{
			...
			modalDialog(
				...
				footer = div(
				actionButton(inputId= ns('confirmRefresh'), label='Yes'),
				modalButton('cancel')
				)
			)
		)
...
dbServer <- function(id, dm) {
  moduleServer(
      ...
      function(input, output, session) {
        ...
        tables <- names(dm::dm_get_tables(dm))
        ...
        servers <- lapply(tables, function(x){
			tableServer(id=paste0('DT_',x), table=x, dm=dm)	
		})
